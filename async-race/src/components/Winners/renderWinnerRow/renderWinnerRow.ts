export function renderWinnerRow(id: number, number: number, color: string, name: string, wins: number, time: number) {
  const table = document.querySelector<HTMLDivElement>('.winners-table');
  const tRow = document.createElement('div');
  tRow.classList.add('table-row');
  tRow.innerHTML = `
    <div class = "col col-1 w-number">${number}</div>
    <div class = "col col-2 w-car">
      <svg class = "win-img win-img-${id}" xmlns="http://www.w3.org/2000/svg" fill = ${color}
    ...
    <div class = "col col-5 w-Best">${time}</div>
  `;
  table?.append(tRow);
}
